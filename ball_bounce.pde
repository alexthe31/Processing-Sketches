...
void setup() {
  ...
  for(int i = 0; i < balls.length; i++){
     balls[i] = new Ball(new PVector(random(width), random(height)), new PVector(random(VEL_RANGE * 2) - VEL_RANGE, random(VEL_RANGE * 2) - VEL_RANGE), random(1, 25), random(1, 15)); 
  }
...
void moveBall(Ball b, float delta) {
  b.position.add(new PVector(b.velocity.x, b.velocity.y).mult(1));
  if(b.position.x <= 0 + b.radius){
    b.position.x = b.radius + 1;
    applyForce(b, new PVector(1, 0).mult(b.velocity.x * b.mass * -2));
  }
  ...
  if(b.position.y <= 0 + b.radius){
     ...
     applyForce(b, new PVector(0, 1).mult(b.velocity.y * b.mass * -2));
  }
...
void mousePressed() {
  //applyForce(b1, new PVector(0, 100));
}
